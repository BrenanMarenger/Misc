...
public class KnightsTourSolver extends PuzzleSolver {
	...
	public List<GameMove> nextLegalMoves() {
		...
		for (int jumpIndex = 0; jumpIndex < 8; jumpIndex++) {
			GameCoordinate possibleCoord = currentCoord.offsetBy(fileOffsets[jumpIndex], rankOffsets[jumpIndex]);
	...
	public boolean isOnBoard(GameCoordinate aCoord) {
		return aCoord.fileIndex >= 0 && aCoord.fileIndex < board.getSize() && aCoord.rankIndex >= 0
				&& aCoord.rankIndex < board.getSize();
	}

	public boolean hasVisited(GameCoordinate aCoord) {
		for (GameMove eachMove : moves) {
			if (eachMove.to().fileIndex == aCoord.fileIndex && eachMove.to().rankIndex == aCoord.rankIndex) {
				return true;
			}
		}
		return false;
